...
export default class App extends Component{
  ...
  handleButtonPress2 = () => {
    const { navigation } = this.props;
    navigation.navigate('Emergence User Massege');
  };
  ...
  fetchData= async()=>{
    const response = await fetch('http://192.168.0.119:3000/Message2');
...
const Chat = () => {
  navigation.navigate("Chat");
    ...
    return (<View style = {{flexDirection:'column',justifyContent : 'center',alignItems :'center'}}>
     <View style={styles.Header}>  
      ...
      <MaterialIcons name='arrow-back' size={26} color='white' style={{marginLeft:20}}/>
       ...
       <Text style= {styles.Font}>Emergence User Massege</Text>
      </View>
    ...
    <View style={styles.cardContainer}>
       ...
       renderItem={({item}) =>
          <MassageFormat date={formatDate(item.Date)} name={item.fromID}  tittle={item.Tittle} to={item.ToID ='All'} content={item.Content}/>
